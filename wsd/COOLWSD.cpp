...
void COOLWSD::initializeEnvOptions()
{
    ...
    while ((aliasGroup = std::getenv(("aliasgroup" + std::to_string(n + 1)).c_str())) != "")
    {
        bool first = true;
    ...
    _overrideSettings["admin_console.username"] = std::getenv("username");
    _overrideSettings["admin_console.password"] = std::getenv("password");
    _overrideSettings["server_name"] = std::getenv("server_name");
    _overrideSettings["allowed_languages"] = std::getenv("dictionaries");
}
